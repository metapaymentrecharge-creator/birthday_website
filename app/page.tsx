...
const removePhoto = (index: number) => {
  ...
  const handlePhotos = async (e: ChangeEvent<HTMLInputElement>) => {
    ...
    // 1. Instant local preview taaki screen par koi glitch ya delay na ho
    const localPreviews = selectedFiles.map((file) => URL.createObjectURL(file));
    setPhotos((prev) => [...prev, ...localPreviews].slice(0, 10));
    setPhotoTransforms((prev) => [
      ...prev,
      ...localPreviews.map(() => ({ x: 0, y: 0, scale: 1 })),
    ]);

    // 2. Background mein Supabase par upload karo aur real public URLs se replace karo
    try {
      ...
      if (uploadedUrls.length > 0) {
        setPhotos((prev) => {
          ...
          return [...filtered, ...uploadedUrls].slice(0, 10);
        });
      }
    } catch (err) {
          ...
          <div className="relative z-10 mx-auto max-w-5xl">
            ...
            <div className="grid gap-5 lg:grid-cols-[1fr_.8fr]">
              <div className="glass-card rounded-[32px] p-5 md:p-7 space-y-6">
                ...
                <div>
  <label className="mb-2 block text-sm font-bold">Born on (Date of Birth) 📅</label>
  <input
    type="date"
    value={dob}
  ...
  />
  <p className="mt-1 text-[10px] text-gray-400">Select date from calendar (Auto-formatted)</p>
</div>
      ...
      {/* ================= PREVIEW ================= */}
      {step === "preview" && (
        <section className="relative min-h-screen overflow-hidden bg-[#fff7fb] px-4 py-8">
          ...
          <div className="relative z-10 mx-auto max-w-5xl">
            <div className="mb-8 text-center">
              <p className="text-xs font-bold uppercase tracking-[0.25em] text-pink-500">STEP 02 · PREVIEW</p>
              <h1 className="mt-3 text-4xl font-black md:text-6xl">This is how their <span className="gradient-text block pb-2">surprise will feel.</span></h1>
            </div>

            <div className="mx-auto max-w-[430px] overflow-hidden rounded-[36px] border border-white bg-white shadow-2xl shadow-purple-200/70">
              <div className="relative min-h-[700px] overflow-hidden p-6" style={{ background: theme.gradient }}>
                <div className="preview-shimmer" />
                <div className="relative z-10">
                  <div className="mb-7 flex items-center justify-between">
                    <div className="rounded-full border border-white/30 bg-white/20 px-3 py-2 text-[9px] font-bold tracking-[0.2em] text-white backdrop-blur-xl">
                      MADE WITH LOVE ({photos.length} Photos)
                    </div>
                    <span className="animate-spin-slow text-xl">✨</span>
                  </div>

                  <div className="preview-photo-wrap overflow-hidden rounded-[30px] border border-white/50 bg-white/20 shadow-2xl">
                    <div className="relative aspect-[4/5] overflow-hidden">
                      {photos[0] && (
                        <>
                          <img src={photos[0]} alt="" className="absolute inset-0 h-full w-full scale-110 object-cover blur-2xl opacity-50" />
                          <div className="absolute inset-0 flex items-center justify-center overflow-hidden">
                            <img src={photos[0]} alt="" className={`h-full w-full object-contain transition duration-[1800ms] ${previewAnimation ? "scale-[1.04] blur-0" : "scale-100"}`} style={photoStyle(0)} />
                          </div>
                        </>
                      )}
                      <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-transparent to-black/10" />
                      <div className="absolute bottom-0 left-0 right-0 p-6 text-white">
                        <p className="text-xs font-bold uppercase tracking-[0.25em] text-white/70">HAPPY BIRTHDAY {dob ? `• ${dob}` : ''}</p>
                        <h2 className="mt-2 text-4xl font-black">{partnerName || "Name"}</h2>
                        <div className="mt-2 flex flex-wrap gap-1">
                          <span className="rounded-full bg-white/20 px-2.5 py-0.5 text-[10px] font-bold backdrop-blur">{ageLine}</span>
                          {relationship && <span className="rounded-full bg-white/20 px-2.5 py-0.5 text-[10px] font-bold backdrop-blur">{relationship}</span>}
                        </div>
                      </div>
                    </div>
                  </div>
                  ...
                  <button
                    ...
                    className="mt-4 w-full rounded-2xl border border-white/40 bg-white/20 py-3 text-xs font-black text-white backdrop-blur transition hover:bg-white/30 cursor-pointer touch-manipulation"
                  >
                    {previewAnimation ? "↻ REPLAY PREVIEW" : "▶ TAP TO PREVIEW ANIMATION"}
                  </button>

                  {/* All Selected Photos Gallery Showcase */}
                  <div className="mt-4 text-left">
                    <p className="text-[9px] font-black uppercase tracking-wider text-white/80 mb-1.5">Selected Photos Gallery ({photos.length}) 📸</p>
                    <div className="flex gap-2 overflow-x-auto pb-1 scrollbar-thin">
                      {photos.map((p, idx) => (
                        <div key={idx} className="relative h-14 w-14 shrink-0 overflow-hidden rounded-xl border border-white/40 shadow-md bg-black/40">
                  ...
                  <div className="mt-3 rounded-2xl bg-black/25 p-3 backdrop-blur-md text-left text-white border border-white/20">
                    <p className="text-[9px] font-black uppercase tracking-wider text-pink-300">🎵 Song: {songName || "Custom Track Selected"}</p>
                    <p className="mt-1 text-xs font-semibold leading-relaxed text-white/95">“{message || "Aapka personal message yahan dikhega..."}”</p>
                  </div>
                </div>
              </div>
            </div>

            <div className="mx-auto mt-7 flex max-w-[430px] flex-col gap-3">
              <button
                type="button"
                onClick={goPayment}
                className="shine-button rounded-2xl bg-gradient-to-r from-pink-500 via-purple-500 to-sky-500 py-5 text-sm font-black text-white shadow-xl shadow-pink-200 transition transform hover:scale-[1.02] active:scale-95 cursor-pointer touch-manipulation"
              >
                ✨ THIS LOOKS PERFECT (PROCEED)
              </button>
              <button
                type="button"
                onClick={() => setStep("form")}
                className="rounded-2xl border border-pink-200 bg-white py-4 text-sm font-black text-pink-600 shadow-sm transition hover:bg-pink-50 cursor-pointer touch-manipulation flex items-center justify-center gap-2"
              >
                ✏️ EDIT DETAILS (Kuch change krna hai?)
              </button>
            </div>
          </div>
        </section>
        ...
        <section className="relative flex min-h-screen items-center overflow-hidden bg-[#fff7fb] px-4 py-8">
          ...
          <div className="relative z-10 mx-auto w-full max-w-md">
            ...
            <div className="overflow-hidden rounded-[32px] border border-white bg-white/90 shadow-2xl shadow-purple-200/60 backdrop-blur-xl p-6">
              ...
              {/* 🔥 Sample Preview Box added back here */}
              <div className="flex items-center gap-4 rounded-2xl bg-gradient-to-r from-pink-50 to-purple-50 p-4 border border-pink-100 mb-6">
                <div className="relative h-20 w-16 overflow-hidden rounded-xl bg-gray-200 shadow-md shrink-0">
                  {photos[0] ? (
                ...
                <div className="min-w-0 flex-1 text-left">
                  <p className="text-[10px] font-bold uppercase tracking-wider text-pink-500">Live Preview Sample</p>
                  <h3 className="truncate text-lg font-black text-gray-800">{partnerName || "Birthday Star"}</h3>
                  <p className="text-xs text-gray-500 truncate">Vibe: {theme.name}</p>
                </div>
              </div>
      ...
      {/* ================= FINAL (WishViewer integrated inside single file) ================= */}
      {step === "final" && (
        <div 
