  ...
  const displayRazorpay = async () => {
    ...
    const options = {
      ...
      handler: async function (response: any) {
        // Yeh block ab sirf tabhi chalega jab payment actual mein SUCCESS ho jayegi
        await goFinal();
      },
      prefill: {
        name: yourName,
  ...
  const goFinal = async () => {
    try {
      ...
      if (data && data.length > 0) {
        ...
        const link = `${window.location.origin}/?id=${insertedId}`;
  ...
  />
      ...
      {showShareModal && (
        <div className="fixed inset-0 z-[110] flex items-center justify-center bg-black/70 p-4 backdrop-blur-md">
          <div className="w-full max-w-md rounded-[32px] bg-white text-[#17121c] shadow-2xl p-6 text-center space-y-5 animate-fade-up">
            <div className="mx-auto flex h-16 w-16 items-center justify-center rounded-full bg-emerald-100 text-3xl">
              🎉
            </div>
            <div>
              <h3 className="text-2xl font-black text-gray-900">Payment Successful!</h3>
              <p className="mt-1 text-xs text-gray-500">Aapka birthday surprise link taiyar hai.</p>
            </div>
            ...
            <div className="rounded-2xl bg-gray-50 p-3 border border-gray-100 flex items-center gap-2">
              <input 
                ...
                readOnly 
                value={shareableLink} 
                className="w-full bg-transparent text-xs font-bold text-gray-700 outline-none px-2"
              />
              <button 
                type="button"
                onClick={() => {
                ...
                className="rounded-xl bg-gray-900 px-4 py-2 text-xs font-black text-white shrink-0 cursor-pointer"
              >
                COPY
              </button>
            </div>
            ...
            <a
              href={`https://wa.me/?text=${encodeURIComponent(`Maine tumhare liye ek special birthday surprise banaya hai! Yahan click karke dekho: ${shareableLink}`)}`}
              target="_blank"
              rel="noopener noreferrer"
              className="flex items-center justify-center gap-3 w-full rounded-2xl bg-[#25D366] py-4 text-sm font-black text-white shadow-lg shadow-emerald-200 transition hover:scale-[1.02] active:scale-95 cursor-pointer"
            >
            ...
            <button
              ...
              className="w-full rounded-2xl bg-gray-100 py-3 text-xs font-bold text-gray-600 hover:bg-gray-200 cursor-pointer"
            >
              Apna Surprise Preview Dekho →
            </button>
          </div>
        </div>
      )}
